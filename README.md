...
Tauri、SolidJS、TypeScriptを用いて開発されたVRゴーグル向け画像ビューア。VRパススルー環境で素早く高精度な画像確認を実現します。
...
- **高速起動**: 最小限の依存で軽量実装
- **直感的操作**: VRコントローラーでの自然なジェスチャー対応
- **モダンUI**: Glassmorphismによる洗練されたインターフェース
...
- **ファイル操作**: ローカルストレージの画像ファイル読み込み（PNG、JPEG、BMP、GIF、WEBP、TIFF、AVIF、JXL等）
...
- `@jsquash/jxl`: JPEG XL画像のWebAssemblyデコーダー
...
### eGUI版（Rust単体）の開発・ビルド

eGUI版はTauri/JavaScriptを使用せず、Rustのみで動作する軽量版です。

#### 開発時の実行（ホットリロード）

`src-tauri` ディレクトリ内で実行してください。
...
※ `cargo-watch` がインストールされていない場合は `cargo install cargo-watch` を実行してください。
...
ビルドされた実行ファイルは `src-tauri/target/release/vdi-egui.exe` に生成されます。
