...
現在の「最新版をチェック」ボタンを改善し、新しいバージョンが利用可能な場合は「今すぐアップデート」ボタンに変更する。ボタンをクリックすると、自動的に最新版がダウンロード・インストールされる。

## 目的

ユーザーがアップデートを簡単にインストールできるようにし、アップデートの手間を削減する。
...
1. **ボタンの動的表示**
   - 初期状態: 「最新版をチェック」ボタンを表示
   - アップデートが利用可能な場合: 「今すぐアップデート」ボタンに変更
   - 最新版を使用している場合: 「最新版をチェック」ボタンを表示

2. **アップデートチェック機能**
   - 「最新版をチェック」ボタンクリック時
     - 最新版をチェック
     - アップデートが利用可能な場合、ボタンを「今すぐアップデート」に変更
     - メッセージでバージョン情報を表示
...
ja.jsonとen.jsonに以下のキーを追加:
- `version.installUpdate`: 「今すぐアップデート」(ja) / "Install Update" (en)
- `version.installing`: 「インストール中...」(ja) / "Installing..." (en)

### UpdateManagerの拡張

`src/services/UpdateManager.ts`に新しいpublicメソッドを追加:
...
既存の`performUpdate`メソッドをpublicに変更し、`installUpdate`として公開する。

### VersionInfoコンポーネントの修正

`src/components/SettingsMenu/VersionInfo.tsx`の変更:

1. **状態の追加**
   - `availableUpdate`: 利用可能なアップデート情報を保存
   - `isInstalling`: インストール中かどうかを示すフラグ

2. **ハンドラーの追加**
   - `handleCheckUpdate`: アップデートチェック処理(既存)
   - `handleInstallUpdate`: アップデートインストール処理(新規)

3. **ボタンの動的表示**
   - `availableUpdate`がnullの場合: 「最新版をチェック」ボタンを表示
   - `availableUpdate`が存在する場合: 「今すぐアップデート」ボタンを表示
   - `isInstalling`がtrueの場合: ボタンを無効化し「インストール中...」と表示
...
2. ボタンが無効化され、「チェック中...」と表示
3. アップデートをチェック
4. アップデートが利用可能な場合:
   - ボタンテキストが「今すぐアップデート」に変更
   - メッセージで新しいバージョンを表示
5. 最新版の場合:
   - ボタンは「最新版をチェック」のまま
   - メッセージで「最新版を使用しています」と表示

### インストールフロー

1. ユーザーが「今すぐアップデート」ボタンをクリック
2. ボタンが無効化され、「インストール中...」と表示
3. アップデートをダウンロード・インストール
4. 完了後、アプリを自動的に再起動
...
1. **状態管理**
   - `availableUpdate`をnullで初期化
   - チェック成功時のみ更新
   - インストール後は状態をリセット(再起動するので不要)

2. **エラーハンドリング**
   - UpdateManagerのエラーメッセージを活用
   - ネットワークエラー、署名エラーなど適切に処理

3. **既存機能との互換性**
   - バックグラウンドアップデートチェック機能は維持
   - 自動ダイアログ表示機能は維持
...
- `src/locales/ja.json` - 日本語翻訳キー追加
- `src/locales/en.json` - 英語翻訳キー追加
- `src/services/UpdateManager.ts` - installUpdateメソッド追加
- `src/components/SettingsMenu/VersionInfo.tsx` - ボタン動的表示実装
