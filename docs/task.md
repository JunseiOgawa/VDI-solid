...
GitHub ActionsとTauriの公式updaterプラグインを使用した完全な自動アップデートシステムの実装。mainブランチへのプッシュで自動ビルド・リリースを行い、アプリ側ではバックグラウンドチェックと手動チェック機能を提供する。

---

## フェーズ1: GitHub Actionsワークフロー

### タスク1.1: 自動リリースワークフローの作成

**目的**: mainブランチへのプッシュで自動的にビルド・リリースを行うワークフローを作成

**ファイル**: `.github/workflows/auto-release.yml`

**作業内容**:
- [ ] ワークフローファイルを新規作成
- [ ] トリガー条件を設定（mainブランチへのpush、ドキュメントファイルは除外）
- [ ] matrix戦略を定義（Windows、Linux）
- [ ] チェックアウトステップを追加
- [ ] Node.jsセットアップステップを追加
- [ ] Rustセットアップステップを追加
- [ ] Rustキャッシュステップを追加
- [ ] Linux依存関係インストールステップを追加
...
### タスク1.2: バージョン自動決定ロジックの実装

**目的**: PATCHバージョンの自動インクリメントとMAJOR/MINORの手動変更を実現

**作業内容**:
- [ ] 最新のGitタグを取得するステップを追加
- [ ] タグからバージョン番号を抽出するロジックを実装
- [ ] package.jsonのバージョンと比較するロジックを実装
- [ ] 手動変更を検出する条件分岐を実装
- [ ] PATCH番号を自動インクリメントするロジックを実装
- [ ] 新しいバージョンをGITHUB_OUTPUTに出力

**検証**:
- バージョン決定ロジックが正しく動作する（シミュレーション）
- 手動変更の検出が正しく動作する

---

### タスク1.3: バージョンファイル更新ロジックの実装

**目的**: package.json、Cargo.toml、tauri.conf.jsonのバージョンを自動更新

**作業内容**:
- [ ] package.jsonを更新するステップを追加（npm version使用）
- [ ] Cargo.tomlを更新するステップを追加（sedコマンド使用）
- [ ] tauri.conf.jsonを更新するステップを追加（sedコマンド使用）
- [ ] Windowsでも動作するようにPowerShellスクリプトも用意

**検証**:
- 各ファイルが正しく更新される（ローカルでテスト）
- sedコマンドの正規表現が正しい

---

### タスク1.4: パッチノート自動生成の実装

**目的**: 前回のタグから今回までのコミットログからパッチノートを生成

**作業内容**:
- [ ] 最新のタグを取得するステップを追加
- [ ] 前回タグから今回までのコミットログを取得
- [ ] コミットログをフォーマットしてパッチノートを生成
- [ ] 初回リリース時の処理を実装
- [ ] マルチライン出力をGITHUB_OUTPUTに対応させる
...
### タスク1.5: Gitタグ作成とプッシュの実装

**目的**: 新しいバージョンのGitタグを作成してリモートにプッシュ

**作業内容**:
- [ ] Gitのユーザー設定を追加（github-actions[bot]）
- [ ] 新しいタグを作成するステップを追加
- [ ] タグをリモートにプッシュするステップを追加
...
### タスク1.6: Tauriビルド・リリースの実装

**目的**: tauri-apps/tauri-actionを使用してビルドとリリースを実行

**作業内容**:
- [ ] npm依存関係インストールステップを追加
- [ ] tauri-actionステップを追加
- [ ] GITHUB_TOKENを環境変数に設定
- [ ] tagName、releaseName、releaseBodyを設定
- [ ] releaseDraftとprereleaseフラグを設定
- [ ] argsでターゲットとcreateUpdaterArtifactsを指定

**検証**:
- ワークフロー全体の構成が正しい
- tauri-actionのパラメータが正しい

---

### タスク1.7: 既存のreleaseワークフローの削除

**目的**: 既存のタグベースリリースワークフローを置き換え

**作業内容**:
- [ ] `.github/workflows/release.yml`を削除（またはリネーム）
- [ ] 削除理由をコミットメッセージに記載
...
**目的**: Cargo.tomlにupdaterプラグインの依存関係を追加

**ファイル**: `src-tauri/Cargo.toml`

**作業内容**:
- [ ] 既存の依存関係を確認
- [ ] desktop専用の依存関係セクションを追加
- [ ] tauri-plugin-updater = "2.0"を追加
- [ ] tauri-plugin-process = "2.0"を追加（既存の場合は確認）
- [ ] ビルドが通ることを確認
...
### タスク2.2: Rust側のプラグイン登録

**目的**: lib.rsでupdaterプラグインを登録

**ファイル**: `src-tauri/src/lib.rs`

**作業内容**:
- [ ] 既存のプラグイン登録を確認
- [ ] setupクロージャーにupdaterプラグインを追加
- [ ] desktop環境でのみ動作するようにcfg属性を設定
- [ ] tauri_plugin_processプラグインを追加（既存の場合は確認）
- [ ] ビルドが通ることを確認
...
### タスク2.3: tauri.conf.jsonの設定

**目的**: updaterプラグインの設定を追加

**ファイル**: `src-tauri/tauri.conf.json`

**作業内容**:
- [ ] bundleセクションにcreateUpdaterArtifacts: trueを追加
- [ ] pluginsセクションを追加
- [ ] updater設定を追加
  - [ ] active: true
  - [ ] endpoints配列を設定（GitHub Releasesのエンドポイント）
  - [ ] dialog: false（自前のダイアログを使用）
  - [ ] pubkey: ""（署名なし）
- [ ] ビルドが通ることを確認

**検証**:
- `npm run build`が成功する
- tauri.conf.jsonの構文が正しい

---

## フェーズ3: フロントエンド実装

### タスク3.1: JavaScript依存関係の追加

**目的**: package.jsonにupdaterプラグインの依存関係を追加

**ファイル**: `package.json`

**作業内容**:
- [ ] @tauri-apps/plugin-updater@^2を追加
- [ ] @tauri-apps/plugin-process@^2を追加（既存の場合は確認）
- [ ] npm installを実行
- [ ] ビルドが通ることを確認
...
### タスク3.2: UpdateManagerサービスの実装

**目的**: アップデート管理ロジックを実装

**ファイル**: `src/services/UpdateManager.ts`（新規作成）

**作業内容**:
- [ ] ファイルを新規作成
- [ ] 必要なimportを追加
- [ ] 定数を定義（チェック間隔、レート制限）
- [ ] UpdateCheckResultインターフェースを定義
- [ ] UpdateManagerクラスを実装
  - [ ] loadLastCheckTimeメソッド
  - [ ] saveLastCheckTimeメソッド
  - [ ] checkForUpdatesBackgroundメソッド
  - [ ] checkForUpdatesManualメソッド
  - [ ] performUpdateCheckメソッド
  - [ ] showUpdateDialogメソッド
  - [ ] performUpdateメソッド
  - [ ] canPerformManualCheckメソッド
  - [ ] recordManualCheckメソッド
  - [ ] getCurrentVersionメソッド
- [ ] updateManagerインスタンスをエクスポート
...
### タスク3.3: vite.config.tsの修正

**目的**: パッケージバージョンを環境変数として埋め込む
...
- [ ] fsモジュールをインポート
- [ ] package.jsonを読み込み
- [ ] defineセクションにPACKAGE_VERSIONを追加
- [ ] ビルドが通ることを確認

**検証**:
- `npm run build`が成功する
- import.meta.env.PACKAGE_VERSIONにバージョン番号が設定される

---

### タスク3.4: Appコンポーネントでの初期化

**目的**: 起動時にアップデートチェックを初期化

**ファイル**: `src/App.tsx`

**作業内容**:
- [ ] UpdateManagerをインポート
- [ ] onMountフックを確認（既存の場合は追加）
- [ ] loadLastCheckTimeを呼び出し
- [ ] checkForUpdatesBackgroundを呼び出し（エラーハンドリング付き）
- [ ] ビルドと動作確認

**検証**:
- `npm run build`が成功する
- アプリ起動時にバックグラウンドチェックが実行される

---

### タスク3.5: VersionInfoコンポーネントの実装

**目的**: バージョン情報画面を実装

**ファイル**: `src/components/Settings/VersionInfo.tsx`（新規作成）
...
### タスク3.6: SettingsMenuへの統合

**目的**: VersionInfo画面を設定メニューに統合

**ファイル**: `src/components/Settings/index.tsx`

**作業内容**:
- [ ] VersionInfoコンポーネントをインポート
- [ ] 設定メニューの適切な位置にVersionInfoを配置
- [ ] レイアウトを調整
- [ ] ビルドと動作確認

**検証**:
- `npm run build`が成功する
- 設定メニューからバージョン情報画面にアクセスできる
...
**目的**: 開発環境でアップデート機能をテスト

**作業内容**:
- [ ] ビルドが成功することを確認
- [ ] UpdateManagerの各メソッドをテスト
  - [ ] loadLastCheckTime
  - [ ] checkForUpdatesManual（モック）
  - [ ] getCurrentVersion
- [ ] VersionInfo画面の動作確認
- [ ] レート制限の動作確認
- [ ] エラーハンドリングの動作確認
...
**目的**: GitHub Actionsワークフローの動作を確認

**作業内容**:
- [ ] 新しいブランチを作成（feature/auto-update）
- [ ] すべての変更をコミット
- [ ] mainブランチにプッシュ（またはプルリクエスト）
- [ ] ワークフローの実行を監視
- [ ] ビルドが成功することを確認
- [ ] リリースが作成されることを確認
- [ ] バージョン番号が正しいことを確認
...
- GitHubのReleaseページに公開される
- Updaterアーティファクトが生成される

---

### タスク4.3: 実際のアップデート動作の検証

**目的**: ビルドされたアプリでアップデート機能をテスト
...
- [ ] 新しいバージョンをリリース
- [ ] 古いバージョンのアプリを起動
- [ ] バックグラウンドチェックが実行されることを確認
- [ ] アップデートダイアログが表示されることを確認
- [ ] アップデートをインストール
- [ ] アプリが再起動され、新しいバージョンになることを確認
- [ ] 手動チェックも確認

**検証**:
- バックグラウンドチェックが正常に動作する
- 手動チェックが正常に動作する
- アップデートのダウンロード・インストールが成功する
- アプリが正常に再起動する
...
- [ ] `.github/workflows/auto-release.yml`が作成されている
...
- [ ] lib.rsでプラグインが登録されている
- [ ] tauri.conf.jsonで設定が完了している
- [ ] ビルドが成功する

### フロントエンド
- [ ] UpdateManagerサービスが実装されている
- [ ] Appコンポーネントで初期化されている
- [ ] VersionInfo画面が実装されている
- [ ] 設定メニューに統合されている
- [ ] バージョン情報が正しく表示される
...
- 開始日: 2025年1月21日
- 完了予定日: 未定
- 現在のフェーズ: フェーズ1（GitHub Actionsワークフロー）

---

## 備考

- **重要**: 各フェーズを順番に完了させてから次のフェーズに進むこと
- ビルドが通ることを確認してから次のタスクに進むこと
- 各フェーズ完了後にユーザーに確認を求めること
- コミットはユーザーの承認を得てから行うこと
- GitHub Actionsワークフローのテストは慎重に行うこと（無限ループを避ける）
...
### 1.1 依存関係のインストール
- [x] `@solid-primitives/i18n` をインストール
- [x] package.jsonに依存関係を追加
- [x] 型定義を確認

### 1.2 ディレクトリとファイル構造の作成
- [x] `src/locales/` ディレクトリを作成
- [x] `src/locales/ja.json` を作成
- [x] `src/locales/en.json` を作成
- [x] `src/locales/index.ts` を作成
...
### 3.1 AppStateContextの修正
- [x] locale signalを追加
- [x] setLocale関数を追加
- [x] localStorageからのロケール読み込み
- [x] localStorageへのロケール保存
- [x] システムロケールのフォールバック

### 3.2 i18n統合
- [x] createI18nContextの設定
- [x] AppStateContextでi18nコンテキストを提供
- [x] 翻訳関数(t)の公開

## フェーズ4: コンポーネントの更新

### 4.1 Titlebarコンポーネント
- [x] aria-labelをt()で置き換え
- [x] titleをt()で置き換え
- [x] 動作確認

### 4.2 Footerコンポーネント
- [x] テキストをt()で置き換え
- [x] ダイアログメッセージをt()で置き換え
- [x] エラーメッセージをt()で置き換え
- [x] 動作確認

### 4.3 SettingsMenuコンポーネント
- [x] メニュー項目をt()で置き換え
- [x] 説明テキストをt()で置き換え
- [x] 言語切り替えUIを追加
- [x] 動作確認

### 4.4 MultiMenuコンポーネント
- [x] GridMenuContentをt()で置き換え
- [x] PeakingMenuContentをt()で置き換え
- [x] HistogramMenuContentをt()で置き換え
- [x] 動作確認

### 4.5 ImageGalleryコンポーネント
- [x] ヘッダーテキストをt()で置き換え
- [x] 並び替えオプションをt()で置き換え
- [x] 動作確認

### 4.6 その他コンポーネント
- [x] VersionInfoコンポーネント
- [x] その他必要なコンポーネント
- [x] 動作確認
...
### 5.1 言語選択UI
- [x] SettingsMenuに言語選択セクションを追加
- [x] ラジオボタンまたはドロップダウンを実装
- [x] 選択された言語の視覚的フィードバック
- [x] 言語切り替え時の即時反映
...
### 1.1 日本語翻訳(ja.json)
- [ ] `version.installUpdate`: "今すぐアップデート"を追加
- [ ] `version.installing`: "インストール中..."を追加

### 1.2 英語翻訳(en.json)
- [ ] `version.installUpdate`: "Install Update"を追加
- [ ] `version.installing`: "Installing..."を追加

## フェーズ2: UpdateManagerの拡張

### 2.1 新しいメソッドの追加
- [ ] `installUpdate(update: Update)`メソッドを追加
- [ ] 既存の`performUpdate`の処理を活用
- [ ] エラーハンドリングを実装
...
## フェーズ3: VersionInfoコンポーネントの修正

### 3.1 状態の追加
- [ ] `availableUpdate`状態を追加(Update | null)
- [ ] `isInstalling`状態を追加(boolean)

### 3.2 ハンドラーの実装
- [ ] `handleCheckUpdate`を修正してavailableUpdateを更新
- [ ] `handleInstallUpdate`を新規作成

### 3.3 UIの動的表示
- [ ] ボタンテキストの動的切り替え実装
- [ ] ボタンクリックハンドラーの動的切り替え実装
- [ ] isInstallingによる無効化処理
...
# LUT（Look-Up Table）適用機能

## フェーズ1: LUTファイル読み込み機能

### 1.1 lutUtils.tsの作成

- [ ] `src/lib/lutUtils.ts`ファイルを作成
- [ ] LutDataインターフェースを定義
- [ ] parseCubeFile関数を実装
  - [ ] ファイル読み込み機能
  - [ ] .cubeファイルのパース処理
...
- [ ] `src/components/ImageViewer/LutLayer.tsx`ファイルを作成
- [ ] LutLayerPropsインターフェースを定義
- [ ] WebGL初期化処理を実装
  - [ ] WebGLコンテキストの取得
  - [ ] シェーダープログラムのコンパイル
  - [ ] 頂点シェーダーの実装
  - [ ] フラグメントシェーダーの実装
- [ ] テクスチャ作成関数を実装
  - [ ] createImageTexture関数
  - [ ] createLutTexture関数
  - [ ] create3DLutTexture関数
- [ ] 描画処理を実装
  - [ ] ユニフォーム変数の設定
  - [ ] テクスチャのバインド
  - [ ] 描画コマンドの実行
- [ ] createEffectによる自動再描画
- [ ] onCleanupによるリソース解放

### 2.2 WebGLヘルパー関数の作成

- [ ] `src/lib/webglUtils.ts`ファイルを作成
- [ ] createShader関数を実装
- [ ] createProgram関数を実装
- [ ] createTexture関数を実装
- [ ] setupQuad関数を実装（フルスクリーン四角形）
...
### 3.1 LutMenuContent.tsxの作成

- [ ] `src/components/ImageViewer/LutMenuContent.tsx`ファイルを作成
- [ ] LutMenuContentPropsインターフェースを定義
- [ ] LUT有効化トグルを実装
- [ ] LUTファイル選択ボタンを実装
- [ ] LUT不透明度スライダーを実装
- [ ] ファイル名表示機能を実装
...
### 3.2 MultiMenuコンポーネントの修正

- [ ] `src/components/ImageViewer/MultiMenu.tsx`を修正
- [ ] LUTタブを追加（tabs配列に追加）
- [ ] LutMenuContentをインポート
- [ ] LUTタブの表示処理を実装
- [ ] LUT関連のpropsを追加
...
### 4.1 AppStateContextの拡張

- [ ] `src/context/AppStateContext.tsx`を修正
- [ ] LUT関連の状態を追加
  - [ ] lutEnabled
  - [ ] lutData
  - [ ] lutOpacity
...
### 4.2 ImageManagerコンポーネントの修正

- [ ] `src/components/ImageViewer/ImageManager.tsx`を修正
- [ ] ImageManagerPropsにLUT関連propsを追加
...
### 4.3 ImageViewerコンポーネントの修正

- [ ] `src/components/ImageViewer/index.tsx`を修正
- [ ] AppStateContextからLUT関連の状態を取得
- [ ] ImageManagerにLUT関連propsを渡す

### 4.4 LUTファイル選択処理の実装

- [ ] AppStateContextにLUTファイル選択ハンドラーを追加
- [ ] selectLutFile関数を呼び出す
- [ ] parseCubeFile関数でファイルをパース
- [ ] LUTデータを状態に保存
- [ ] エラーハンドリング
...
- [ ] 不正なLUTファイルのエラーハンドリング
- [ ] WebGL非対応環境のエラーハンドリング
- [ ] ファイル読み込みエラーのハンドリング
- [ ] エラーメッセージの表示

### 5.5 コードフォーマット

- [ ] npx prettier で全ファイルをフォーマット
- [ ] コードレビュー
- [ ] コメントの追加・修正
...
- [ ] ビルドエラーがない
- [ ] 型エラーがない
- [ ] レイヤー順序が「画像→LUT→ピーキング→グリッド」になっている
- [ ] LUT適用が正常に動作する
- [ ] パフォーマンスが許容範囲内
